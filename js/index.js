$('#message').countdown('2020/12/21', { elapse: true }).on('update.countdown', function(event) {
	if(event.elapsed) $(this).text(event.strftime('<h2>코코아가 성인이 되고...</h2><div id="counter"><div id="value"><h1>%-m개월 %-d일<br>%-H시간 %M분 %S초</h1></div></div>지났습니다.'));
	else $(this).html('<h1>' + event.strftime('\<h2>코코아가 성인이 될 때까지...</h2><div id="counter"><div id="value"><h1>%-m개월 %-d일<br>%-H시간 %M분 %S초</h1></div></div>남았습니다.' + '</h1>'));
});
